...
const CallListItem: React.FC<CallListItemProps> = ({ call, onUpdateCall, onCreateCall, onSelectCall, selectedMember = '全体', isHighlighted, isRecentlyUpdated = false, isNewCall = false, showRequesterColumn, members, users, isPrecheckTheme = false, isElecTheme = false, currentUser, isDuplicate, isDarkMode = false }) => {
  ...
  const { liStyle, dateTimeStyle, absenceCounterClass } = useMemo((): { liStyle: React.CSSProperties; dateTimeStyle: React.CSSProperties; absenceCounterClass: string; } => {
    const defaultMainTextClass = isElecTheme ? 'text-[#2d5a9e]' : isPrecheckTheme ? 'text-[#118f82]' : 'text-[#0193be]';
    if (call.status === '完了') {
      return {
        liStyle: {
  ...
  const mainTextClass = isElecTheme ? 'text-[#2d5a9e]' : isPrecheckTheme ? 'text-[#118f82]' : 'text-[#0193be]';
  const mainRingClass = isElecTheme ? 'ring-[#2d5a9e]' : isPrecheckTheme ? 'ring-[#118f82]' : 'ring-[#0193be]';
      ...
      {isNewCall && (
        <>
          ...
          <div style={{
            ...
            background: isElecTheme ? '#2d5a9e' : isPrecheckTheme ? '#118f82' : '#0193be',
      ...
      <div className="px-2 py-1 flex items-center gap-1.5 text-sm">
          ...
          <div className="w-28 flex-shrink-0 flex items-center gap-1 group">
              ...
              {isCopied ? (
                  ...
                  <button onClick={handleCopy} className="p-1 text-current/60 rounded hover:bg-slate-300/50 hover:text-current/80 transition duration-300 flex-shrink-0" title="顧客IDをコピー">
            ...
            <div className="w-20 flex-shrink-0 flex items-center justify-center">
              {call.prechecker ? (
                ...
                <div className="relative h-5 w-5">
                  <button
                    className={`cursor-pointer h-5 w-5 rounded-full border-2 border-slate-400 bg-white transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                        isElecTheme ? 'focus:ring-[#2d5a9e]' : isPrecheckTheme ? 'focus:ring-[#118f82]' : 'focus:ring-[#0193be]'
                    } disabled:opacity-50 disabled:cursor-not-allowed`}
                    disabled={!(hasPrecheckPermission || hasElecPermission) || isCompleted}
                    onClick={handlePrecheckerClick}
          ...
          <div className="w-8 flex-shrink-0 flex items-center justify-center">
              <button
                  ...
                  className="p-1.5 text-current/60 rounded-full hover:enabled:bg-slate-200/60 hover:text-current/80 transition duration-300 disabled:cursor-not-allowed"
                  title="この依頼を編集する"
              >
