...
const App: React.FC = () => {
  ...
  const handleToggleLike = async (userName: string) => {
    ...
    try {
      ...
      console.error('いいねの更新に失敗しました:', err?.message ?? err);
    }
  };
                ...
                <div className="my-4">
                  {displayViewMode === 'precheck' || (displayViewMode === 'others' && selectedMember === PRECHECKER_ASSIGNEE_NAME)
                     || displayViewMode === 'elec' || (displayViewMode === 'others' && selectedMember === ELEC_ASSIGNEE_NAME) ? (
                      ...
                      return (
                      <div
                          className={`rounded-xl overflow-hidden transition-colors duration-500 ${mineIsAvailable ? (isDarkMode ? 'bg-white/5 border border-[#0193be]/25' : 'bg-white/70 border border-[#0193be]/20') : ''}`}
                              ...
                              <div className="flex-1 min-w-0">
                                  {currentUserWithData?.comment ? (
                                      <div className="mb-2">
                                          ...
                                          {/* 自分のコメントへのいいね表示 */}
                                          {(() => {
                                            const myLikers = commentReactions.filter(r => r.userName === currentUser.name).map(r => r.reactor);
                                            const isLikedByMe = myLikers.includes(currentUser.name);
                                            return (
                  ...
                  : displayViewMode === 'others' && selectedMember !== '新規依頼' && selectedMember !== '全体' && (() => {
                          ...
                          return (
                              <div
                                  className={`rounded-xl overflow-hidden transition-colors duration-500 ${isAvailable ? (isDarkMode ? 'bg-white/5 border border-[#0193be]/25' : 'bg-white/70 border border-[#0193be]/20') : ''}`}
                                  style={gradientStyle}
                              >
                                  {/* 上部：プロフィール + 基本情報 */}
                                  <div className="flex items-center gap-4 px-5 pt-5 pb-4">
                                      ...
                                      <div className="flex-1 min-w-0">
                                          {selectedUserDetails.comment && (
                                              <div className="mb-2">
                                                  ...
                                                  {/* 他者コメントへのいいね表示 */}
                                                  {(() => {
                                                    const theirLikers = commentReactions.filter(r => r.userName === selectedMember).map(r => r.reactor);
                                                    if (!selectedMember) return null;
                                                    const isLikedByMe = theirLikers.includes(currentUser.name);
                                                    return (
                                                      <div className="flex flex-wrap gap-1 mt-1">
                                                        <button
                        ...
                        return (
                          <div key={previewMember} className="animate-wipe-in-down-slow">
                            <div className="mb-4">
                              ...
                              <div
                                  className={`rounded-xl overflow-hidden transition-colors duration-500 ${pvIsAvailable ? (isDarkMode ? 'bg-white/5 border border-[#0193be]/25' : 'bg-white/70 border border-[#0193be]/20') : ''}`}
